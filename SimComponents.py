...
class WeightedRoundRobin(object):
    ...
    def __init__(self,env,ports,weights,mean):
        ...
        self.env = env
        self.n_ports = ports
        self.port_weights = weights
        self.mean_pkt_size = mean
        self.outs = [None for i in range(self.n_ports)]
        self.packets_rec = 0
        self.packets_to_be_served = []

        self.normalized_weight = [value / self.mean_pkt_size for value in self.port_weights]

        self.min_normalized_weight = min(self.normalized_weight)
    ...
    def put(self, pkt):
        ...
        if self.serving_list[self.serving_port] == 0:
            self.serving_port += 1

        if self.serving_list[self.serving_port] > 0:
            if self.outs[self.serving_port]:
                self.outs[self.serving_port].put(pkt)
...
class LeastConnection(object):
    ...
    def __init__(self, env, ports, endpoints):
        self.env = env
